...
def build_metapath(df):
    ...
    # 合并二三级类目
    df['category23'] = df.apply(lambda x: merge_category(x.category2, x.category3), axis=1)
    ...
    top_brand_num = int(len(brand_cnt) * percent)
    top_brand_lst = brand_cnt.most_common(top_brand_num)
    print("Top {}% coverage - {}".format(percent * 100, sum([x[1] for x in top_brand_lst]) / sum(brand_cnt.values())))
    ...
    print("Top {}% coverage - {}".format(percent * 100, sum([x[1] for x in top_cate_lst]) / sum(cate_cnt.values())))
...
def build_features(df, dataset_name):
    ...
    if os.path.exists(feature_file):
        ...
        item_num = df['asin'].max()
        df['rank'] = df['rank'].apply(extract_rank, args=(rank_set, rank_max))
        # 提取price
        df['price'] = df['price'].apply(extract_price)

        df['rank_main'] = df['rank'].apply(gen_main_rank)
        df['rank_sub'] = df['rank'].apply(gen_sub_rank)
