...
def func_train(args, file_paths, gpu, ngpus_per_node):
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_train, num_workers=1,
                              collate_fn=my_fn)
    valid_loader = DataLoader(valid_set, batch_size=args.batch_train, shuffle=False, num_workers=1, collate_fn=my_fn)
    ...
    features = build_features(df).to(args.device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        #                              args, logger)
        train_loss = new_train_epoch(model, optimizer, train_loader, hg, features, args, logger)
        if args.local_rank in [-1, 0]:
